...
tagged_bits_to_bytes::sptr tagged_bits_to_bytes::make(
    std::string key, bool little_endian, int stub_mode, int v_len, double sample_rate)
{
...
void tagged_bits_to_bytes_impl::forecast(int noutput_items,
...
void tagged_bits_to_bytes_impl::set_vlen(int vlen)
{
    d_vlen = vlen;
    d_itemsize = 8 * vlen;
}

void tagged_bits_to_bytes_impl::set_little_endian_flag(bool lsb_first)
{
    d_lsb_first = lsb_first;
}

void tagged_bits_to_bytes_impl::set_sample_rate(double samp_rate)
{
    d_sample_rate = samp_rate;
}


uint16_t tagged_bits_to_bytes_impl::packbits(uint8_t* bits, uint32_t length)
{
    uint16_t packed = 0;
...
size_t tagged_bits_to_bytes_impl::packbuffer(const uint8_t* in,
                                             size_t length,
                                             uint8_t* out_array,
                                             size_t nbytes_align)
{
    ...
    if (d_stub_mode == DROP_STUB) {
        ...
        nbytes +=
            (nbytes_align - (nbytes % nbytes_align)) % nbytes_align; // round upbytes
    }
...
int tagged_bits_to_bytes_impl::general_work(int noutput_items,
                                            ...
                                            gr_vector_void_star& output_items)
{
    ...
    // if there is at least a single tag, consume up until it, leave the rest for the next
    // work() call
    ...
    for (auto tag : alltags) {
        ...
        if (pmt::equal(tag.key, PMTCONSTSTR__rx_time())) {
            ...
            pmt::pmt_t updated_time =
                pmt::make_tuple(pmt::from_uint64(int_sec), pmt::from_double(frac_sec));
    ...
    if ((rel_offset != 0) and (tags.size()) and (tags[0].offset == abs_idx)) {
        ...
        pmt::pmt_t time_pair_pmt = pmt::make_tuple(pmt::from_uint64(this_time.sec),
