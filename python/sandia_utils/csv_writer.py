...
class csv_writer(gr.sync_block):
    ...
    def message_handler(self, msg):
        ...
        elif pmt.is_pdu(msg) or pmt.is_pair(msg):
            car = pmt.car(msg)
            cdr = pmt.cdr(msg)
            pass
        else:
            self.log.debug("PMT is neither dict nor pair")
        ...
        try:
            ...
            for i in range(len(fields)):
                ...
                if values[0] in self.metadata_mappings:
                    ...
                    def print_func(x, values=values): return str(self.metadata_mappings[values[0]](x))

                    if len(values) == 2:
                        spec = '{:' + values[1] + '}'
                        def print_func(x, spec=spec, values=values): return spec.format(self.metadata_mappings[values[0]](x))
