    ...
    def handler(self, msg):
        if pmt.is_dict(msg):
            ...
            if not pmt.is_dict(pmt.car(msg)):
                self.log.debug("pair must have a dict as first element")
                return
            meta = pmt.car(msg)
        else:
            self.log.debug("pmt must be either dict or a pair")
            return

        # attempt to extract file name
        fpmt = pmt.dict_ref(meta, self.filename_tag, pmt.PMT_NIL)
