...
class csv_reader(gr.sync_block):
    ...
    def __init__(self, fname='', has_header=True, data_type='uint8',
                 period=1000, start_delay=0, repeat=True, key_list=[]):
    ...
    def load_header(self):
        ...
        if self.has_header:
            try:
                ...
                if self.key_list:
                    # If keylist specified set header with only specified data
                    # set header
                    self.header = [(pmt.intern(keys[keys.index(k)]), self.metadata_mappings[values[keys.index(k)]])
                                   if k in self.key_list else () for k in keys]
                    for val in self.key_list:
                        if val not in keys:
                            print("Warning: '" + val + "' key not found")
